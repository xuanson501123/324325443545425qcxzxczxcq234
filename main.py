...
def git_commit_and_push(msg: str):
    ...
    subprocess.run(
        [str(x) for x in ["git", "config", "--global", "user.name", user]],
        check=True)
    subprocess.run([
        str(x) for x in [
            ...
            f"{user}@users.noreply.github.com"
                   ...
                   check=True)
    subprocess.run(
        [str(x) for x in ["git", "remote", "set-url", "origin", remote_url]],
        check=True)
    try:
        ...
        print("⚠️ Lỗi stash/pull. Tiếp tục...")
    try:
...
def delete_udid_by_value(udid: str) -> bool:
    data = load_udid_data()
    if udid in data:
        del data[udid]
        save_udid_data(data)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message:
        await update.message.reply_text(
            "✅ Gửi UDID để thêm hoặc chọn thời hạn bằng nút ➖➕⏪⏩.")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not udid:
        ...
        return
    if query.data and query.data.startswith("increase_"):
        step = int(query.data.split("_")[1])
        context.user_data["days"] = min(MAX_DAYS, days + step)
    elif query.data and query.data.startswith("decrease_"):
        step = int(query.data.split("_")[1])
        context.user_data["days"] = max(1, days - step)
    elif query.data == "confirm":
        old_expiry, new_expiry = extend_expiry(udid, context.user_data["days"])
        msg = (
            f"🔄 UDID {udid} đã tồn tại\n📅 Hạn cũ: {old_expiry}\n➡️ Hạn mới: {new_expiry}"
            if old_expiry else
            f"✅ Đã duyệt UDID mới: {udid}\n📅 Hạn dùng: {new_expiry}")
        msg += "\n⏱️ Chờ 3-5 phút có thể sử dụng Mod."
        msg += "\nVideo hướng dẫn sử dụng mod Youtube: @tiptipmodios."
        await query.edit_message_text(msg)
        return
    elif query.data == "delete_udid":
        ...
        return
    # Cập nhật message sau khi tăng/giảm ngày
